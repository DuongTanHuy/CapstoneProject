...
contract BlindAuction {
    ...
    function participateInAuction(uint _id, uint _bid) public {
        ...
        require(
            msg.sender != auctionCreators[_id],
            "Auction creator cannot participate in their own auction"
        );
        require(
            !auctionParticipants[_id][msg.sender],
            "You have already participated in this auction"
        );
    ...
    function getAllAuctions() public view returns (AuctionInfo[] memory) {
        AuctionInfo[] memory auctionsList = new AuctionInfo[](
            auctionIds.length
        );
