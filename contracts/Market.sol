...
contract Market {
    ...
    /// @dev start_price is the price of the item
    ...
    /// @param start_price Price of the item listed.
    ...
    ) external payable condition(start_price > 0) {
    ...
    /// @dev the buyer confirms to get the refund of the money owed.(start_price)
    /// @dev the beneficiary gets his amount + start_price (3 * start_price)
        ...
        CheckState(auction_id)
    {
        ...
        Listings[auction_id].beneficiary.transfer(
            3 * Listings[auction_id].start_price
        );
        /// Refund the buyer
        Listings[auction_id].buyer.transfer(Listings[auction_id].start_price);
        activelistings -= 1;
